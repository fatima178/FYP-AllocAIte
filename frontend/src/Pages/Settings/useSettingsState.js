...
// convert skill rows from the form into the shape expected by the backend
...
const buildHistoryTitle = (item) => {
  // history can come from either a saved assignment title or a free-text task
...
const buildCollapsedHistoryTitle = (item) => {
  // keep long task titles short when cards are collapsed
...
const buildTopMatchesLabel = (item) => {
  // show only the first few ranked recommendations in the collapsed history view
...
const validateWeights = (nextWeights, managerWeightTotal) => {
  // frontend validation gives the user feedback before sending to the backend
...
const expandGroupedWeights = (groupedWeights, config) => {
  // UI shows grouped categories, but the matching code stores detailed weights
...
const collapseDetailedWeights = (detailedWeights, config) => {
  // convert stored detailed weights back into grouped slider values
...
export function useSettingsCoreState() {
  // account, appearance, weightings, export and password state for settings page
  ...
  const updateSettings = useCallback(async (payload, successMessage = "Settings updated") => {
    // shared save helper for small settings changes
  ...
  useEffect(() => {
    // load settings once when the settings page opens
    const userId = getSessionItem("user_id");
    if (!userId) {
  ...
  useEffect(() => {
    // apply visual changes to the page immediately
  ...
  const remainingWeightPoints = useMemo(
    // sliders work in whole percentage points, so this shows what is left to allocate
    () =>
      Math.max(
        0,
  ...
  const updateWeightAllocation = (key, nextPoints) => {
    // clamp the selected slider so all categories cannot go above the total budget
  ...
  const saveWeights = () => {
    // weights must add up before saving, otherwise recommendations would be skewed
  ...
  const exportAllData = async () => {
    // export downloads a workbook generated by the backend
    const userId = getSessionItem("user_id");
    if (!userId) {
